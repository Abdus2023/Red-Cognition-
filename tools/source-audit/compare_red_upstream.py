...
GENERATED_AUDIT_PREFIXES = {"tools/source-audit/"}


def include_in_upstream_comparison(path: str) -> bool:
    return path not in GENERATED_AUDIT_PATHS and not any(path.startswith(prefix) for prefix in GENERATED_AUDIT_PREFIXES)
...
def tracked_head_files() -> list[str]:
    ...
    return [p for p in (out.splitlines() if out else []) if include_in_upstream_comparison(p)]
...
def main() -> None:
    ...
    data = {
        ...
        "generated": {
            ...
            "timestamp_utc": _dt.datetime.now(_dt.timezone.utc).isoformat(),
            "tool": TOOL,
            "method": "compare SHA-256 of local HEAD tracked files, excluding generated audit artifacts, against locally fetched upstream Red v0.6.4 tag; no source files modified",
        },
        "repository": {
