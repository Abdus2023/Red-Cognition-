...
rfc_ids={re.search(r'RFC-(\d{4})',f.name).group(0) for f in (ROOT/'rfcs').glob('RFC-*.md')}
missing_rfcs=sorted(x for x in rfc_ids if f'](../rfcs/{x}-' not in rfc_text)
...
result={'indexed_files':len(indexed),'expected_files':len(expected),'missing_files':missing,'stale_files':stale,'rfcs':len(rfc_ids),'missing_rfcs':missing_rfcs,'wiki_pages':len(wiki_paths),'missing_wiki_pages':missing_wikis,'result':'PASS' if not errors else 'FAIL','errors':errors}
(DOCS/'repository-index-validation.json').write_text(json.dumps(result,indent=2)+'\n')
print('REPOSITORY INDEX VALIDATION\n===========================')
for k in ('indexed_files','expected_files','rfcs','wiki_pages'): print(f'{k.replace("_"," ").title()+":":<24} {result[k]}')
