...
## Message #23 additions — compile-time and distributed security (msg#23 [221]–[240])

- **Ratified capability/security governance (RFC-0050, [224]):** no cognitive process may bypass capability authorization, policy enforcement, or audit recording; security chain Identity → Capability → Policy → Sandbox → Execution → Audit ratified; separation of reasoning/authorization/execution capability ([222]).
- **Compile-time capability model (RFC-0051 §6, [227]):** macro execution MUST be capability-gated; a macro MUST NOT access filesystem/network, execute arbitrary external code, or modify external state without explicit capability; all capability usage recorded in the expansion trace; prevents "compiler plugins become unrestricted privileged programs" ([228]).
- **Verification security (RFC-0052 §3.5, [229]/[233]):** capability usage validation, policy compliance checking, sandbox isolation testing; Capability and Policy verification categories.
- **Remote invocation security (RFC-0053 §9–10, [237]/[239]):** all messages MUST be authenticated; sensitive payloads MAY be encrypted; replay protection MUST be employed; mutual authentication MAY be required for high-trust interactions; receiving agent MUST verify capabilities against local policy (RFC-0025); capability violations MUST produce traceable errors in the event log.

## Message #25 additions — federation and transaction security (msg#25 [241]–[260])

- **CADFP security model ([249] §14):** all registration and discovery operations MUST be authenticated; cross-domain operations MUST carry verifiable trust assertions; federation events MUST be integrity-protected; discovery/registration capability-gated; policies in CSPL (RFC-0025).
- **CSMKSP security ([253]):** access to shared memory capability-gated; subscriptions capability-gated; synchronization events recorded in the unified event log; privacy-preserving shared knowledge listed as open question.
- **CDTCP security posture ([257]/[259]/[260]):** transaction participation and effects capability-gated; capability revocation → immediate abort (failure matrix); v1.2 leaves authentication/authorization/replay protection/commit authorization/coordinator & participant identity implicit — review [260] flags a dedicated security section as a remaining normative gap (not incorporated in corpus).
- **CRAIP v1.2 retained security model ([241] §12):** authenticated messages, optional payload encryption, mandatory replay protection, optional mutual authentication for high-trust interactions.

## Message #26 additions — transaction security and trust (msg#26 [261]–[280])

- **CDTCP v1.3 security ([261]/[265] §19):** integrates RFC-0022/RFC-0025; coordinator election MUST be authenticated; transaction integrity MUST be protected; replay protection tokens MUST be employed; coordinator and participant identities MUST be verified. Capability revocation → immediate abort (failure matrix).
- **CTWP v1.2 security ([275]):** integrity/auth optional block per message; flags Authenticated/Encrypted/Compressed/ReplayProtected; ReplayProtection MUST for distributed deployments; duplicate suppression via MessageID/sequence; cryptographic profiles proposed in reviews ([272]: Minimal CRC32C / Secure SHA-256+Ed25519 / Enterprise SHA-512+post-quantum).
- **CTSTP v1.0 ([279]) + v1.1 proposal ([280]):** cryptographic identity for every node/agent/CVM (public-key based, bound to NodeID/AgentID/CVMID, verifiable across domains); messages across untrusted boundaries MUST carry cryptographic hashes; integrity verified before processing; replay detection MUST reject replays and log events; trust = identity verification + capability authorization + policy evaluation + attestation; secure channels MUST provide confidentiality/integrity/replay protection with authenticated establishment; least privilege principle; v1.1 proposal adds trust chains (Root → Domain → Runtime → Agent → Participant), signature coverage (CDTPEnvelope + Payload + Sequence Information), secure channel profiles (TCP+TLS, QUIC, Authenticated IPC, Secure Message Queue), attestation support, security failure matrix.

## Message #27 additions — ratified security plane and instruction-level enforcement (msg#27 [281]–[300])

- **CTSTP ratified (RFC-0059, [280]/[291]):** normative security guarantees — identity verification, message integrity, authentication, capability authorization, replay resistance, trust-chain evaluation, security auditing, deterministic security decisions, attestation support, security event replay. Cryptographic algorithm profiles proposed (Profile A SHA-256/SHA-3 + Ed25519 + X25519; Profile B SHA-3-512 + Dilithium + Kyber for quantum resistance, [290]). Security decision determinism via SecurityDecisionHash.
- **Instruction-level security (RFC-0060/0061, ratified):** capability check + transaction check before execute; SecurityViolation aborts instruction and generates trace event; attestation enforced before hardware-accelerated instructions (RFC-0026); effect-producing instructions MUST declare capability/effect-class/security-level.
- **Bytecode security (RFC-0062, draft):** hash verification, format validation, capability check, policy verification before execution; signed bytecode for Enterprise profile; proof-carrying bytecode for Verified profile.

---

## Message #29 additions — bytecode signing, artifact trust, and deployment security (msg#29 [301]–[320])

> Provenance: corpus message #29 ([301]–[320]); verbatim archives `sources/message-029-original-part1..5.md`; scaffolds in `rfcs/`.

- **Bytecode security requirements** (RFC-0062, [303] §16 / [304] §12): before loading bytecode the CVM MUST verify magic → version → integrity → signature → capabilities → verification metadata → execution; unsigned or invalid bytecode MUST NOT execute in secure or verified profiles. v1.1 adds a normative verification pipeline (Load→Check Magic→Check Version→Verify Integrity→Validate Sections→Validate Types→Validate Control Flow→Validate Capabilities→Validate Effects→Executable), domain-separated canonical hashing HashDomain { FormatID, Version, SectionID, Payload } (e.g. HASH("CVMX-CODE-v1" + CodeSection)) preventing cross-domain hash collisions, and SignatureBlock { Algorithm, PublicKeyID, Signature, CertificateChain } required for Enterprise/Verified profiles ([304] §13/§15).
- **Artifact trust classification** (RFC-0065 clarifications, [310]): ArtifactTrustLevel UNVERIFIED (hash only) / SIGNED (digital signature) / VERIFIED (compiler certificate) / FORMALLY_VERIFIED (machine-checked proofs) / ATTESTED (hardware/runtime attestation); capability manifests gain CapabilityConstraints, DelegationPolicy, EffectPermissions, MaximumAuthority — "This prevents an artifact from gaining more authority after deployment"; ProofBundle bundles CompilerCorrectnessProof, OptimizationProofs[], TypeSafetyProof, CapabilitySafetyProof, EffectSafetyProof, ReplayEquivalenceProof (connecting RFC-0032 COVF, RFC-0063 CVM-FOS, RFC-0064 CCC-VTP).
- **Distribution trust chain** (RFC-0066, [311]/[312]): registries MUST verify CPCPF cryptographic integrity, proof validity (RFC-0032), capability-manifest consistency (RFC-0006), and trust-policy compliance (RFC-0025) before publication; trust = Identity + Proof + Attestation + Audit History (explicitly "not reputation"); federation MUST preserve artifact hash, publisher identity, verification proofs, revocation state, and provenance chain — "A registry cannot 'rewrite' another registry's artifact"; CPRDP completes the CTSTP security chain: RFC-0059 Security Identity → RFC-0065 Proof-Carrying Artifact → RFC-0066 Trusted Distribution → RFC-0042 Autonomous Deployment; pre-execution sequence Identity Verify→Artifact Verify→Proof Verify→Capability Verify→Policy Verify→Execute ([312]).
