...
| **CVMB** | CVM bytecode magic number 0x43564D42 ("CVMB"), first four bytes of a CVM bytecode image per RFC-0062 v1.0 draft [288]; the msg#29 drafts [302]–[304] use CVMX 0x43564D58 instead — divergence C-18 preserved. | [288]; cf. [302]–[304] |
...
| **CVMX** | CVM bytecode magic number 0x43564D58 ("CVMX") in RFC-0062 v1.0 variants [302]/[303] and v1.1 [304] (container, hash domains "CVMX-CODE-v1"); diverges from CVMB in [288] — C-18. | [302], [303], [304] |
| **ModuleIdentity** | RFC-0062 v1.1 canonical bytecode identity { ModuleID, Namespace, Version, CompilerID, SourceHash, BytecodeHash } — logical program id vs exact executable representation vs provenance ([304] §3). | [304] |
| **CVM-FOS (RFC-0063)** | Cognitive Virtual Machine Formal Operational Semantics — mathematical CVM foundation: labeled transition system M=(S,I,→,S₀), step function, small-step rules, Functional Determinism and Replay Equivalence theorems, machine invariants, Lean 4 formalization target; v1.1 Candidate [306]. | [305], [306] |
| **CCC-VTP (RFC-0064)** | Cognitive Compiler Correctness and Verified Translation Pipeline — formal correctness layer: verified pipeline CLS→CIR→COIL→COVF→CISA→encoding→CPCPF; semantic/capability/effect/determinism preservation theorems; transformation certificates; v1.0 Draft [307]; v1.1 amendments recommended [308]. | [307], [308] |
| **CPCPF (RFC-0065)** | Cognitive Proof-Carrying Artifact Format — self-contained machine-verifiable deployment bundle (CISA bytecode, CIR, optimization history/certificates, capability/effect manifests, trace metadata, integrity/attestation); acronym shared with RFC-0033 CPCPF (concept origin) — C-11 naming lineage; v1.0 Draft [309]; 7 ratification clarifications [310]. | [309], [310] |
| **ArtifactTrustLevel** | RFC-0065 ratification-clarification trust classification UNVERIFIED / SIGNED / VERIFIED / FORMALLY_VERIFIED / ATTESTED with hash→signature→compiler certificate→machine-checked proofs→attestation requirements ([310] §3). | [310] |
| **CPRDP (RFC-0066)** | Cognitive Package Registry and Distribution Protocol — operational registry/distribution rules: Package Index, Artifact Storage, Verification Service, Trust Database, Audit Log; deterministic publication/discovery/dependency resolution; revocation; federation; v1.0 Draft [311]; expansions [312]. | [311], [312] |
| **PackageID (CPRDP)** | Immutable cryptographic package identity = Hash(CPCPF artifact + dependency graph + compiler version + proof set); pkg://redcognition/… URIs; hash is the true identity, name secondary ([312]). | [312] |
| **CPM-WS (RFC-0067)** | Cognitive Package Manager and Workspace Specification — developer-facing tooling: workspace tree (cog.toml/cog.lock), manifests, deterministic dependency resolution, lockfiles, build reproducibility, workspace policies, mirrors/offline, `cog` CLI; title echoes ratified RFC-0047 CPMWS (documented overlap); v1.0 Draft [313]. | [313] |
| **CBS-RAP (RFC-0068)** | Cognitive Build System and Reproducible Artifact Pipeline — deterministic build engine: Build(InputHash)=ArtifactHash, hermetic execution, BuildGraph DAG, BuildProfile, content-addressed cache, BuildMetadata, distributed BuilderNode, BuildReceipt, build events; v1.0 Draft [314] (CHATGPT). | [314] |
| **BuildReceipt** | CBS-RAP artifact produced by every successful build { BuildID, WorkspaceHash, ArtifactHash, CompilerHash, VerificationHash, BuilderIdentity, CompletionEvent }; becomes part of artifact provenance ([314] §10). | [314] |
| **CRDLMP (RFC-0069)** | Cognitive Runtime Deployment and Lifecycle Management Protocol — runtime lifecycle inside CogOS: Created→Verified→Registered→Approved→Deployed→Activated→Monitored→Updated/Migrated→Suspended/Rolled Back→Retired/Archived; DeploymentManifest; migrations under RFC-0057 transaction boundaries; v1.0 Draft [315]; 8 v1.1 additions recommended [316]. | [315], [316] |
| **CognitiveDeploymentUnit** | Recommended CRDLMP v1.1 abstraction { DeploymentID, ArtifactID, AgentID, RuntimeInstanceID, CVMVersion, CapabilityProfile, ResourceProfile, LifecycleState, CheckpointReference, ProvenanceChain } bridging static CPCPF artifact → deployment unit → runtime instance ([316] §1). | [316] |
| **LifecycleState (CRDLMP)** | Recommended CRDLMP v1.1 formal state machine CREATED/VERIFIED/REGISTERED/APPROVED/STAGED/DEPLOYING/RUNNING/PAUSED/DEGRADED/MIGRATING/ROLLING_BACK/TERMINATING/RETIRED with constrained transitions ([316] §2). | [316] |
| **CROFP (RFC-0070)** | Cognitive Runtime Orchestration and Federation Protocol — orchestration layer above CRDLMP: Cognitive Runtime Cluster, Orchestration Task, Federation Policy, Deployment Plan, Autonomous Evolution Decision; multi-runtime scheduling, capability federation, checkpoint coordination, policy sync, scaling/self-healing; v1.0 Draft [317]; review [318] marks it the natural stopping point of the core runtime & execution infrastructure. | [317], [318] |
| **CRCP (RFC-0071)** | Cognitive Runtime Coordination Protocol — wire-level control plane for CROFP: capability-gated runtime discovery, orchestration message families (discovery/orchestration/lease/heartbeat/topology/failure recovery), lease expiration semantics, topology sync with causal metadata, deterministic distributed scheduling decisions; v1.0 Draft [319]; 10 pre-ratification areas [320]. | [319], [320] |
| **CRCP Wire Format (RFC-0072)** | Cognitive Runtime Coordination Protocol Wire Format and Binary Message Encoding — transport-agnostic binary control-plane protocol beneath RFC-0071: unified framing/envelope (magic 0x43524350 "CRCP"), message type registry 0x0001…0x00FF (+reserved 0x8000–0x8FFF experimental, 0x9000–0xFFFF private), flag registry, ClientHello/ServerHello version negotiation, encoding profiles (Canonical Binary default), StreamID multiplexing, sequence ordering, replay protection, ErrorMessage schema, deterministic little-endian serialization; v1.6 Ratified [339]. | [321]–[339] |
| **CRCPEnvelope** | RFC-0072 canonical message envelope { MagicNumber, ProtocolVersion, MessageType, Flags, MessageID, SourceNodeID, TargetNodeID, Epoch, SequenceNumber, TraceContext, PayloadLength, Payload, IntegrityBlock }; unified with the wire frame in v1.5+ with explicit field types (uint8/uint16/uint32/uint64, UUIDv7 MessageID, UUID128 node IDs). | [323], [333], [335] |
| **ClientHello / ServerHello (CRCP)** | RFC-0072 handshake messages: ClientHello { SupportedMajorMin/Max, SupportedMinorMin/Max, SupportedFeatures, SupportedEncodings, NodeID } → ServerHello { SelectedVersion, SelectedEncoding, SelectedSecurityProfile, SessionID }; VersionNegotiationFailed error when no mutual version exists. | [333], [335] |
| **TraceContext (CRCP)** | RFC-0072 wire trace metadata { TraceID (UUID128), SpanID (uint64), ParentSpanID (uint64), ReplaySessionID (UUID128), CorrelationID (UUID128) } linking CRCP messages to the RFC-0018 unified event log. | [333], [335] |
| **IntegrityBlock (CRCP)** | RFC-0072 message integrity structure { AlgorithmID (uint16), HashLength (uint16), Hash, SignatureAlgorithm (uint16), SignatureLength (uint16), Signature } — algorithm registries deferred to future RFCs ([336]/[338]). | [333], [335] |
| **ErrorMessage (CRCP)** | RFC-0072 structured error payload { ErrorCode (uint16), Severity (uint8), Message (UTF-8), RelatedMessageID (UUID128), Retryable (bool) }; codes 0x0001 UnknownNode … 0x0007 VersionNegotiationFailed. | [333], [335] |
| **MessageSequence / ReplayProtection (CRCP)** | RFC-0072 ordering and replay models: MessageSequence { SourceNodeID, TargetNodeID, Epoch, SequenceNumber } (monotonic; duplicates ignored; gaps trigger recovery) and ReplayProtection { Nonce, SequenceNumber, Epoch, SessionID }. | [323], [335] |
| **StreamID (CRCP)** | RFC-0072 stream-multiplexing identifier (uint32); stream 0 reserved for control messages; stream lifecycle/flow-control semantics flagged as open by reviews [336]/[338]. | [333], [335], [336] |
| **CRCP Encoding Profiles** | RFC-0072 payload encodings: 0x01 Canonical Binary Encoding (default, MUST-support for interoperability per [339]), 0x02 Deterministic CBOR, 0x03 Deterministic MessagePack, 0x04 Canonical JSON; determinism required within a profile ([330]). | [333], [335], [339] |
| **Registry Governance (RFC-0072 record)** | Ratified-record rule: new assignments to RFC-0072 protocol registries (Message Types, Flags, Encoding Profiles, Error Codes) SHALL require either a subsequently ratified RFC or approval by the designated protocol registry authority ([349]–[359]). | [349], [353] |
| **Protocol Evolution Policy (RFC-0072 record)** | Versioning governance: major increments MAY introduce incompatible wire-format changes; minor increments MUST remain backward compatible within the major version; experimental features MUST remain within reserved registry ranges; deprecated fields remain reserved until a future major revision ([349]–[359]). | [349], [353] |
| **Change Control (RFC-0072 record)** | Post-ratification rule: substantive modifications to RFC-0072 SHALL require a new revision or superseding RFC; editorial corrections that do not alter normative behavior MAY be issued as errata ([351]–[359]). | [351], [353] |
| **Conformance Profile — CRCP Minimal Interoperable Implementation** | RFC-0072 record baseline: implement ClientHello/ServerHello negotiation; support Canonical Binary Encoding; implement deterministic serialization; validate replay protection metadata; recognize all mandatory message types; reject unsupported major protocol versions ([349]–[359]). | [349], [353] |
| **Provisional Ratification (deferred effectiveness)** | Governance pattern introduced by the RFC-0072 record: a child RFC declared Ratified but effective only upon ratification of its still-Candidate parent (RFC-0071); alternatives noted by [360]: "Ratified (Pending Parent Ratification)" / "Provisionally Ratified". | [349], [360] |
| **CVM Machine Model (CISA-RA [369])** | Divergent RFC-0061 v1.0 machine model: CVM { Register File, Operand Stack, Local Memory, Shared Memory Interface, Effect Buffer, Transaction Context, Security Context, Trace Context }; R0–R31 general-purpose registers (Register { Type, Value, Version, Provenance }); special registers PC/SP/FP/TX/CAP/TRACE/EPOCH/FLAGS; cognitive registers BR0–BR7/GR0–GR7/MR0–MR7; three memory spaces (Local no-tx / Working optional / Shared required) ([369]; archived variant, D-102). | [369] |
| **InstructionVerificationFailed** | CISA-RA [369] verification rule: invalid instructions MUST generate `InstructionVerificationFailed` after the Decode→Opcode Validation→Operand Validation→Capability Check→Transaction Check→Execute pipeline. | [369] |
| **SectionEntry / SectionDirectory (CVM-BF)** | RFC-0062 v1.2+ section directory record: SectionEntry { SectionID (2 bytes), Offset (8 bytes), Length (4 bytes), Flags (2 bytes), Hash (32 bytes SHA-256) }; v1.3: entries sorted by SectionID ascending; [380] amendments: SectionDirectory { SectionCount, SectionEntry[] }, unique IDs, no overlap, unknown-critical-section rejection ([375]–[380]). | [375], [377], [380] |
| **CVM-BF Loader Validation Pipeline** | Normative 11-step loading sequence per [380]: Read Header → Validate Magic → Validate Format Version → Validate Section Directory → Validate Section Hashes → Validate Integrity Block → Validate Runtime Compatibility → Validate Capability Manifest → Validate Effect Manifest → Validate Bytecode → Execute; capabilities MUST be verified before the first instruction. | [380] |
| **IntegrityBlock (CVM-BF)** | RFC-0062 integrity structure per [380] amendment: IntegrityBlock { AlgorithmID, ContainerHash, SignatureType, Signature, CertificateReference }; minimum algorithm SHA-256; Enterprise/Verified profiles MAY require signatures, certificate chains, attestation proofs. | [380] |
| **CVM-BF Canonical Constant Encoding** | Per [380] amendment: Constant { Type:uint8, Length:uint32, Value }; Integer → signed two's complement little-endian; Float → IEEE-754; String → UTF-8; UUID → RFC-compatible 128-bit; Hash → raw digest bytes. | [380] |
| **Trusted Computing Base (RFC-0064)** | CCC-VTP TCB: trusted = formal semantics kernel, proof checker, minimal verifier, cryptographic primitives; untrusted = optimizer, compiler frontend, code generator, package sources; principle "the verifier, not the compiler, becomes the root of trust" ([389] §6, ratified per [391]). | [389], [391] |
| **TransformationCertificate** | CCC-VTP per-pass certificate (msg#33 form): CertificateID/CompilerVersion/InputArtifactHash/OutputArtifactHash/PassIdentifier/TransformationRule/Preconditions/ProofObligation/VerificationResult/CapabilityDelta/EffectDelta/ProvenanceChain — enables independent compiler verification, reproducible builds, supply-chain auditing ([388]; ratified requirement per [391]). | [388], [391] |
| **CPCAVP (RFC-0065)** | Cognitive Proof-Carrying Artifact Format and Verification Protocol (msg#33 lineage of RFC-0065, supersedes msg#29 CPCPF form — C-21): CPCA-magic container bundling source manifest, CIR, proof certificates, CISA, CVM bytecode, capability/effect manifests, security attestation, replay metadata, integrity block; zero-trust verification model ([395], ratified RFC-0064 parent). | [394], [395] |
| **ArtifactIdentity (CPCAVP)** | RFC-0065 [395] identity model: ArtifactID (UUID128), ContentHash/SourceHash/CIRHash/BytecodeHash/ProofHash (SHA-256), CompilerID, CompilerVersion; any transformation invalidates identity unless the proof chain is updated ([396]). | [395], [396] |
| **ProofCertificate (CPCAVP)** | RFC-0065 [395] proof bundle: SemanticProof, TypeSafetyProof, CapabilityProof, EffectProof, DeterminismProof, VerificationKernelID ([395]; refinement ProofGraph proposed [396]). | [395], [396] |
| **CARTDP (RFC-0066)** | Cognitive Artifact Registry and Trust Distribution Protocol (msg#33 lineage of RFC-0066, supersedes msg#29 CPRDP form — C-21): Artifact Index/Storage/Verification Service/Trust Database/Audit Ledger; deterministic ArtifactID-based resolution; trust = verification evidence, not reputation; federation preserves identity/proofs/revocation ([397]). | [397], [398] |
| **CDLMP (RFC-0067)** | Cognitive Deployment and Lifecycle Management Protocol (msg#33 lineage of RFC-0067, supersedes msg#29 CPM-WS form — C-21): lifecycle Created→Verified→Registered→Approved→Deployed→Activated→Monitored→Updated/Migrated→Suspended/Rolled Back→Retired/Archived; DeploymentManifest; RFC-0057-bounded migrations ([399]). | [399], [400] |
| **CRGAOP (proposed)** | Cognitive Runtime Governance and Autonomous Operations Protocol — RFC-0068 proposal in [400] (autonomous policy enforcement, runtime decision authority, resource arbitration, agent supervision, safety constraints); numbering collides with the msg#29 RFC-0068 CBS-RAP scaffold (C-21 pattern). | [400] |
| **Cognitive Software Supply Chain** | Milestone declared in [400]: complete verified lifecycle Intent→Language→Compiler→Proof→Artifact→Registry→Deployment→Execution→Observation→Evolution across RFC-0061…0067 (ratified 0061–0064 + draft 0065–0067). | [400] |
