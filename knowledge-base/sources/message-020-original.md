# Source Record: Conversation Message #20 (Verbatim)
...
- **Duplicate status:** identical duplicate of message #1 (governing extraction specification); logged as D-63. No supersession: message #1 remains the origin of record.
...
*Rendering artifact cleanup applied (no semantic changes): the title arrived as doubly-encoded HTML entity `&amp;amp;` (decoded to `&`); redundant blank lines, trailing two-space line breaks, and split list/numbered-item formatting introduced by copy/paste were normalized — the same artifact classes cleaned in message #1. Received-vs-cleaned diff is rendering-only; word-for-word content is identical to message #1 (programmatic comparison: archived bodies byte-exact equal). No semantic content altered; message #1 remains the governing origin of record (D-63).*
