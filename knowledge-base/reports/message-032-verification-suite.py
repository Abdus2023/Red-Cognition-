#!/usr/bin/env python3
"""Message #32 final verification suite for the Red-Cognition knowledge base.
Run from the repository root: python3 knowledge-base/reports/message-032-verification-suite.py
Corpus scope: KB messages 1..32 processed, sub-messages [1]..[380], SN-001..SN-2198,
...
TRANSCRIPT_MSGS = [2, 3, 5, 8, 10, 12, 14, 16, 18, 21, 22, 23, 25, 26, 27, 29, 30, 31, 32]
MSG_PARTS = {2: 2, 3: 4, 5: 5, 8: 5, 10: 5, 12: 5, 14: 5, 16: 5, 18: 5, 21: 5, 22: 5, 23: 5, 25: 5, 26: 5, 27: 5, 29: 5, 30: 5, 31: 5, 32: 5}
EXPECTED_RANGES = {2: (1, 20), 3: (21, 40), 5: (41, 60), 8: (61, 80), 10: (81, 100),
                   12: (101, 120), 14: (121, 140), 16: (141, 160), 18: (161, 180),
                   21: (181, 200), 22: (201, 220), 23: (221, 240), 25: (241, 260), 26: (261, 280), 27: (281, 300), 29: (301, 320), 30: (321, 340), 31: (341, 360), 32: (361, 380)}
SN_RANGES = {2: (1, 123), 3: (124, 212), 5: (213, 318), 8: (319, 427), 10: (428, 493),
             12: (494, 640), 14: (641, 825), 16: (826, 993), 18: (994, 1093),
             21: (1094, 1138), 22: (1139, 1229), 23: (1230, 1348), 25: (1349, 1419), 26: (1420, 1591), 27: (1592, 1777), 29: (1778, 1975), 30: (1976, 2094), 31: (2095, 2122), 32: (2123, 2198)}
FENCED = {2: 120, 3: 89, 5: 106, 8: 109, 10: 66, 12: 147, 14: 185, 16: 168, 18: 100,
          21: 45, 22: 91, 23: 119, 25: 71, 26: 172, 27: 186, 29: 198, 30: 119, 31: 28, 32: 76}
...
check(1, 'all archive files exist (93 files)', not missing, str(missing))
...
check(1, 'sub-message labels [1]..[380] contiguous (380/380)', labels == list(range(1, 381)), 'count=%d' % len(labels))
check(1, 'every sub-message header carries a speaker label', speaker_ok)
check(1, 'D-101 verified in archive: [361]=[363]=[365], distinct from [353]',
      sub_body(32, 361) == sub_body(32, 363) == sub_body(32, 365) and sub_body(32, 361) != sub_body(31, 353))
check(1, 'D-104 verified in archive: [377]=[379], distinct from [375]; [369] distinct from msg#27 [286] (D-102)',
      sub_body(32, 377) == sub_body(32, 379) and sub_body(32, 377) != sub_body(32, 375)
      and sub_body(32, 369) != sub_body(27, 286))
...
check(2, 'SN-001..SN-2198 all present in Code-Snippets', tok_ok, str(miss[:5]))
tot = re.search(r'\*\*Corpus totals: (\d+) snippets\*\*', cs)
check(2, 'Code-Snippets corpus totals line = 2198', tot and tot.group(1) == '2198', tot.group(1) if tot else 'missing')
...
check(2, 'totals line has exactly one parenthetical breakdown (no duplicate residue)',
      tot_line.count('). (') == 0 and tot_line.endswith('Message #32 Annex at the bottom of this page).'),
...
for m in (16, 18, 21, 22, 23, 25, 26, 27, 29, 30, 31, 32):
...
check(2, 'archive fenced total = 2195 (2198 - 3 inline msg#2)', len(all_blocks) == 2195, str(len(all_blocks)))
fenced_ok, fdet = True, []
for m in TRANSCRIPT_MSGS:
    if len(archive_blocks[m]) != FENCED[m]:
        fenced_ok = False; fdet.append('msg%d:%d!=%d' % (m, len(archive_blocks[m]), FENCED[m]))
check(2, 'per-message fenced counts = breakdown sums (all 19 transcript messages)', fenced_ok, str(fdet))
seq_ok, det = True, []
for m in (16, 18, 21, 22, 23, 25, 26, 27, 29, 30, 31, 32):
    lo, hi = SN_RANGES[m]
...
check(2, 'annex SN sequences complete & ascending (msg#16/18/21/22/23/25/26/27/29/30/31/32)', seq_ok, str(det))
annex_ok, bad = True, []
for m in (3, 5, 8, 10, 12, 14, 16, 18, 21, 22, 23, 25, 26, 27, 29, 30, 31, 32):
    hdr = re.search(r'## Message #%d Annex' % m, cs)
...
check(2, 'all annex blocks byte-faithful vs archives (msgs 3..32, 2195 blocks)', annex_ok, str(bad))
...
check(4, '2195/2195 archived fenced blocks verbatim in Wiki', missn == 0,
...
need = ['message-%03d-report.md' % i for i in range(1, 33)]
have = set(os.path.basename(r) for r in reps)
check(4, 'reports message-001..032 exist', all(n in have for n in need), str(len(reps)))
...
# ---------- 5. Normative consistency (message-#32 material) ----------
rec72 = open('rfcs/RFC-0072-ratification-record.md', encoding='utf-8').read()
check(5, 'RFC-0072 record scaffold: [361] publication form (Status qualifier + corrected parentheses + governance sections)',
      '**Status:** **Ratified** (Effective upon ratification of RFC-0071)' in rec72
      and 'is hereby ratified as a normative specification' in rec72
      and 'Canonical envelope structure (`CRCPEnvelope`)' in rec72
      and '(`ClientHello` / `ServerHello`)' in rec72
      and '### Registry Governance' in rec72 and '### Change Control' in rec72)
check(5, 'RFC-0072 record provenance: origin [361]; D-101 + X-140 closure + approvals [362]/[364]/[366]/[368] noted',
      'sub-message [361]' in rec72[:3400] and 'D-101' in rec72[:3400] and 'X-140' in rec72[:3400]
      and '[368]' in rec72[:3400])
check(5, 'RFC-0072 record body matches archive sub-message [361] exactly',
      rec72[rec72.find('-->')+3:].strip() in archive_text[32] or norm(rec72[rec72.find('-->')+3:].strip()) in norm(archive_text[32]))
r62 = open('rfcs/RFC-0062-cvm-bf-bytecode-format-encoding.md', encoding='utf-8').read()
check(5, 'RFC-0062 scaffold: v1.3 per [377] (Candidate for Final Ratification; CVMX; SectionEntry sorted by SectionID)',
      '**Version:** 1.3' in r62 and 'Candidate for Final Ratification' in r62[:1800]
      and '0x43564D58' in r62 and 'sorted by `SectionID` in ascending order' in r62)
check(5, 'RFC-0062 provenance: D-103/D-104 + same-label divergence + [380] ACCEPT + no USER ratification record noted',
      'D-103' in r62[:3600] and 'D-104' in r62[:3600] and 'Ratification Recommendation: ACCEPT' in r62[:3600]
      and 'No USER ratification record present in corpus' in r62[:3600])
check(5, 'RFC-0062 scaffold body matches archive sub-message [377] exactly',
      r62[r62.find('-->')+3:].strip() in archive_text[32] or norm(r62[r62.find('-->')+3:].strip()) in norm(archive_text[32]))
check(5, 'RFC-Index: Message #32 additions section; ratified set unchanged; RFC-0062 row not RATIFIED',
      'Message #32 additions' in ri and 'Ratified set after message #32' in ri
      and all('RATIFIED' not in r for r in re.findall(r'^\| RFC-0062 \|.*$', ri, re.M)))
check(5, 'C-19 extended in conflict log to [361]–[365]',
      'msg#32 [361]–[365]' in st and 'publication form [361]–[365]' in st)
check(5, 'duplicates D-101..D-104 and cross-references X-141..X-144 recorded',
      all(('| D-%d |' % d) in st for d in range(101, 105)) and all(('| X-%d |' % x) in st for x in range(141, 145)))
terms32 = ['CVM Machine Model (CISA-RA [369])', 'InstructionVerificationFailed', 'SectionEntry / SectionDirectory (CVM-BF)',
           'CVM-BF Loader Validation Pipeline', 'IntegrityBlock (CVM-BF)', 'CVM-BF Canonical Constant Encoding']
check(5, 'Glossary contains message-#32 terms (6/6)', all(x in gl for x in terms32),
      str([x for x in terms32 if x not in gl]))

# ---------- 6. RFC parent-chain integrity ----------
no_parent = [f for f in rfc_docs if '**Parent:**' not in open('rfcs/' + f, encoding='utf-8').read()[:4000]]  # widened window: RFC-0062's msg#32 provenance block is longer than the original 1500-char scan
...
check(7, 'README: 32 messages processed; current totals (2198 / 12 specs / 86 rfcs)',
      '32 messages processed' in readme and all(s in readme for s in [
          '**2198 code snippets**', '**12 scaffolded documents in `specs/`**', '**86 files in `rfcs/`**']))
check(7, 'README: no stale messages-processed counts (29/30/31) remain',
      all(('%d messages processed' % n) not in readme for n in (29, 30, 31)))
check(7, 'README: message ordering #30 -> #31 -> #32 with totals ascending',
      readme.index('Message #30 = ') < readme.index('Message #31 = ') < readme.index('Message #32 = ')
      and readme.index('**2122 code snippets**') < readme.index('**2198 code snippets**'))
check(7, 'README Code Snippets table row current (2198 snippets, SN-001…SN-2198)',
      'ledger of all 2198 snippets (SN-001…SN-2198)' in readme and 'ledger of all 2122 snippets' not in readme)
check(7, 'Code-Snippets provenance header current (covers messages #2..#32)',
      '#30, #31, #32' in cs[:700] and 'message-032-part1..5' in cs[:1300])
miss_cl = [i for i in range(1, 33) if ('Message #%d' % i) not in cl]
check(7, 'changelog entries for messages 1..32', not miss_cl, str(miss_cl))

# ---------- 8. Traceability bookkeeping ----------
reg = sorted(set(int(m) for m in re.findall(r'^\| (\d+) \| 2026-', st, re.M)))
check(8, 'register rows 1..32 contiguous', reg == list(range(1, 33)), str(reg))
idx_count = len(re.findall(r'^#{2,3} Message #\d+ sub-message index', st, re.M))
check(8, 'sub-message indexes for the 19 transcript messages', idx_count == 19, str(idx_count))
xs = sorted(set(int(m) for m in re.findall(r'^\| X-(\d+) \|', st, re.M)))
check(8, 'cross-references contiguous X-01..X-144', xs == list(range(1, 145)), 'count=%d' % len(xs))
ds = sorted(set(int(m) for m in re.findall(r'^\| D-(\d+) \|', st, re.M)))
check(8, 'duplicate log contiguous D-1..D-104', ds == list(range(1, 105)), 'count=%d' % len(ds))
