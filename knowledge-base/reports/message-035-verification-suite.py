#!/usr/bin/env python3
"""Deep audit suite #8 (KB message #35, directive "Deeply Verification") for the Red-Cognition knowledge base.
Run from the repository root: python3 knowledge-base/reports/message-035-verification-suite.py
Corpus scope: KB messages 1..35 processed, sub-messages [1]..[420], SN-001..SN-2460,
12 specs + 92 rfcs scaffolded files (104 scaffolds). Deep focus: messages #32-#34
(the three content messages since deep audit suite #7)."""
...
# ---------- 1. Archive structure (incl. duplicate-group byte-identity spot checks) ----------
...
check(1, 'identical re-send groups verified byte-identical in archive (D-99 [361]=[363]=[365]; D-100 [365]=[367]=[369]-group [353]=[355]=[357]=[359]; D-104 [377]=[379]; D-96 [327]=[329]; D-97 [335]=[337])',
      sub_body(32, 361) == sub_body(32, 363) == sub_body(32, 365)
      and sub_body(31, 353) == sub_body(31, 355) == sub_body(31, 357) == sub_body(31, 359)
      and sub_body(32, 377) == sub_body(32, 379)
      and sub_body(30, 327) == sub_body(30, 329) and sub_body(30, 335) == sub_body(30, 337))
check(1, 're-purposed-number pairs verified distinct in archive (D-107/D-108/D-109 msg#29 vs msg#33; D-110/D-111/D-112 msg#29 vs msg#34)',
      sub_body(29, 309) != sub_body(33, 395) and sub_body(29, 311) != sub_body(33, 397) and sub_body(29, 313) != sub_body(33, 399)
      and sub_body(29, 314) != sub_body(34, 401) and sub_body(29, 315) != sub_body(34, 403) and sub_body(29, 317) != sub_body(34, 405))
...
check(3, 'ratification records = 17 (0001,0002,0011,0042,0049,0050,0052,0053,0057..0064,0072)',
      rec_ids == [1, 2, 11, 42, 49, 50, 52, 53, 57, 58, 59, 60, 61, 62, 63, 64, 72], str(rec_ids))
check(3, 'msg#34 renames present; superseded filenames absent (0068/0069/0070)',
      ...
      and not os.path.exists('rfcs/RFC-0065-cpcpf-proof-carrying-artifact-format.md')
      and not os.path.exists('rfcs/RFC-0066-cprdp-package-registry-distribution.md')
      and not os.path.exists('rfcs/RFC-0067-cpm-ws-package-manager-workspace.md'))
...
need = ['message-%03d-report.md' % i for i in range(1, 36)]
have = set(os.path.basename(r) for r in reps)
check(4, 'reports message-001..035 exist', all(n in have for n in need), str(len(reps)))
...
# ---------- 5. Normative consistency — deep focus on messages #32-#34 ----------
rec72 = open('rfcs/RFC-0072-ratification-record.md', encoding='utf-8').read()
check(5, 'msg#32: RFC-0072 record publication form [361] (effectiveness qualifier + corrected parentheses)',
      'Ratified** (Effective upon ratification of RFC-0071)' in rec72 and '(`CRCPEnvelope`)' in rec72 and 'D-101' in rec72[:2400])
r63 = open('rfcs/RFC-0063-cvm-fos-formal-operational-semantics.md', encoding='utf-8').read()
check(5, 'msg#33: RFC-0063 provenance — RATIFIED per [385], divergent v1.0 [384] (D-105) noted, body [306]',
      'RATIFIED per ratification record [385]' in r63[:2600] and 'D-105' in r63[:2600]
      and (r63[r63.find('-->')+3:].strip() in archive_text[29] or norm(r63[r63.find('-->')+3:].strip()) in norm(archive_text[29])))
r64 = open('rfcs/RFC-0064-ccc-vtp-compiler-correctness-verified-translation.md', encoding='utf-8').read()
check(5, 'msg#33: RFC-0064 scaffold v1.1 [389] — TCB section + RATIFIED per [391] + D-106 noted',
      'Trusted Computing Base (TCB)' in r64 and 'RATIFIED per ratification record [391]' in r64[:3400] and 'D-106' in r64[:3400])
r62 = open('rfcs/RFC-0062-cvm-bf-bytecode-format-encoding.md', encoding='utf-8').read()
check(5, 'msg#32/#33: RFC-0062 scaffold [377] lineage + provenance reflects msg#33 ratification per [381]',
      'sub-message [377]' in r62[:2400] and 'RATIFIED per USER ratification record [381]' in r62[:2600])
r61 = open('rfcs/RFC-0061-cisa-ra-instruction-set-register-architecture.md', encoding='utf-8').read()
check(5, 'msg#32: RFC-0061 provenance notes divergent v1.0 [369] (D-102), ratified scaffold unchanged [299]',
      'D-102' in r61[:2600] and 'sub-message [299]' in r61[:2400])
r65 = open('rfcs/RFC-0065-cpcavp-proof-carrying-artifact-verification.md', encoding='utf-8').read()
check(5, 'msg#33: RFC-0065 scaffold CPCAVP [375] — CPCA magic + C-21/D-107 + parent quirk',
      'CPCA' in r65 and 'C-21' in r65[:3000] and 'D-107' in r65[:3000] and 'v1.1 (Candidate)' in r65[:3000])
...
check(5, 'msg#33: RFC-0066 CARTDP [397] (D-108 + citation quirk) and RFC-0067 CDLMP [399] (D-109 + CRGAOP collision note) scaffolded',
      'D-108' in r66[:3200] and 'RFC-0034 — CPRDP' in r66 and 'D-109' in r67[:3200] and 'CRGAOP' in r67[:3200])
...
check(5, 'msg#33: ratification records [381]/[385]/[391] all Ratified + hereby ratified + C-20 noted',
      all('**Status:** **Ratified**' in r and 'is hereby ratified as a normative specification' in r and 'C-20' in r[:2400]
          for r in (rec62, rec63, rec64)))
...
check(5, 'msg#34: RFC-0068 CRGAOP [401] (D-110), RFC-0069 Decision Ledger [403] (D-111), RFC-0070 CRSOAEP [405] (D-112)',
      'D-110' in r68[:3200] and 'D-111' in r69[:3200] and 'D-112' in r70[:3200])
r71 = open('rfcs/RFC-0071-crcp-runtime-coordination-protocol.md', encoding='utf-8').read()
r72d = open('rfcs/RFC-0072-crcp-wire-format-binary-message-encoding.md', encoding='utf-8').read()
check(5, 'msg#34: RFC-0071 CRCP retained (D-113/C-22) body [319]; RFC-0072 ratified retained (D-114/C-22) body [335]',
      'D-113' in r71[:3600] and 'C-22' in r71[:3600]
      and (r71[r71.find('-->')+3:].strip() in archive_text[29] or norm(r71[r71.find('-->')+3:].strip()) in norm(archive_text[29]))
      and 'D-114' in r72d[:3600] and 'C-22' in r72d[:3600]
      and (r72d[r72d.find('-->')+3:].strip() in archive_text[30] or norm(r72d[r72d.find('-->')+3:].strip()) in norm(archive_text[30])))
...
check(5, 'msg#34: RFC-0073 [411], RFC-0074 [415], RFC-0075 v1.1 [419] (CADFP quirk + [420] flag)',
      'v1.0 (Draft)' in r73[:3200] and 'Sovereign' in r74 and 'Candidate for Ratification' in r75[:3200] and 'CADFP' in r75[:3200])
check(5, 'RFC-Index: Message #32/#33/#34 sections present; ratified set sections after #32/#33/#34',
      all(s in ri for s in ('Message #32 additions', 'Message #33 additions', 'Message #34 additions',
                            'Ratified set after message #32', 'Ratified set after message #33', 'Ratified set after message #34')))
check(5, 'no RATIFIED claims for draft numbers (0065-0071, 0073-0075); RFC-0062..0064 + 0072 rows RATIFIED',
      all(not any('RATIFIED' in r for r in re.findall(r'^\| RFC-%04d \|.*$' % n, ri, re.M)) for n in list(range(65, 72)) + [73, 74, 75])
      and all(any('RATIFIED' in r for r in re.findall(r'^\| RFC-%04d \|.*$' % n, ri, re.M)) for n in (62, 63, 64, 72)))
check(5, 'conflicts C-20/C-21/C-22 and duplicates D-105..D-114 recorded',
      '| C-20 |' in st and '| C-21 |' in st and '| C-22 |' in st and all(('| D-%d |' % d) in st for d in range(105, 115)))
check(5, 'cross-references X-145..X-158 present (msgs #32-#34)', all(('| X-%d |' % x) in st for x in range(145, 159)))
terms_deep = ['CRGAOP (RFC-0068, msg#34 lineage)', 'CRDLMP Decision Ledger (RFC-0069, msg#34 lineage)',
              ...
              'Trusted Computing Base (RFC-0064)', 'CPCAVP (RFC-0065)', 'CARTDP (RFC-0066)', 'CDLMP (RFC-0067)']
check(5, 'Glossary contains msgs #32-#34 terms (13/13)', all(x in gl for x in terms_deep),
      str([x for x in terms_deep if x not in gl]))
...
check(7, 'README: 35 messages processed; current totals (2460 / 12 specs / 92 rfcs)',
      '35 messages processed' in readme and all(s in readme for s in [
          '**2460 code snippets**', '**12 scaffolded documents in `specs/`**', '**92 files in `rfcs/`**']))
check(7, 'README: no stale messages-processed counts (32/33/34) remain',
      all(('%d messages processed' % n) not in readme for n in (32, 33, 34)))
...
check(7, 'README Code Snippets table row current (2460 snippets)',
...
miss_cl = [i for i in range(1, 36) if ('Message #%d' % i) not in cl]
check(7, 'changelog entries for messages 1..35', not miss_cl, str(miss_cl))

# ---------- 8. Traceability bookkeeping ----------
reg = sorted(set(int(m) for m in re.findall(r'^\| (\d+) \| 2026-', st, re.M)))
check(8, 'register rows 1..35 contiguous', reg == list(range(1, 36)), str(reg))
