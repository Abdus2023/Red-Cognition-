#!/usr/bin/env python3
"""Message #21 final verification suite for the Red-Cognition knowledge base.
Run from the repository root: python3 knowledge-base/reports/message-021-verification-suite.py
Corpus scope: messages 1..21 processed, sub-messages [1]..[200], SN-001..SN-1138,
12 specs + 51 rfcs scaffolded files."""
...
TRANSCRIPT_MSGS = [2, 3, 5, 8, 10, 12, 14, 16, 18, 21]
MSG_PARTS = {2: 2, 3: 4, 5: 5, 8: 5, 10: 5, 12: 5, 14: 5, 16: 5, 18: 5, 21: 5}
EXPECTED_RANGES = {2: (1, 20), 3: (21, 40), 5: (41, 60), 8: (61, 80), 10: (81, 100),
                   12: (101, 120), 14: (121, 140), 16: (141, 160), 18: (161, 180), 21: (181, 200)}
SN_RANGES = {2: (1, 123), 3: (124, 212), 5: (213, 318), 8: (319, 427), 10: (428, 493),
             12: (494, 640), 14: (641, 825), 16: (826, 993), 18: (994, 1093), 21: (1094, 1138)}
...
check(1, 'all archive files exist (48 files)', not missing, str(missing))
...
check(1, 'sub-message labels [1]..[200] contiguous (200/200)', labels == list(range(1, 201)), 'count=%d' % len(labels))
...
check(2, 'SN-001..SN-1138 all present in Code-Snippets', tok_ok, str(miss[:5]))
tot = re.search(r'\*\*Corpus totals: (\d+) snippets\*\*', cs)
check(2, 'Code-Snippets corpus totals line = 1138', tot and tot.group(1) == '1138', tot.group(1) if tot else 'missing')
for m in (16, 18, 21):
...
check(2, 'archive fenced total = 1135 (1138 - 3 inline msg#2)', len(all_blocks) == 1135, str(len(all_blocks)))
...
check(2, 'annex SN sequences complete & ascending (msg#16/18/21)', seq_ok, str(det))
# byte-exact annex vs archive for all annex messages
annex_ok, bad = True, []
for m in (3, 5, 8, 10, 12, 14, 16, 18, 21):
    lo, hi = SN_RANGES[m]
    hdr = re.search(r'## Message #%d Annex' % m, cs)
...
check(2, 'all annex blocks byte-faithful vs archives (msgs 3..21)', annex_ok, str(bad))
...
check(3, 'rfcs/ = 51 files', len(rfcs) == 51, str(len(rfcs)))
...
check(3, 'RFC-0001..0047 exactly once each', ids == list(range(1, 48)), '%d docs' % len(rfc_docs))
...
check(3, 'all 63 scaffolds carry KB provenance headers', prov_ok)
check(3, 'all 63 scaffold bodies verbatim from archive', faithful == 63,
...
check(4, '1135/1135 archived fenced blocks verbatim in Wiki', missn == 0,
...
need = ['message-%03d-report.md' % i for i in range(1, 22)]
have = set(os.path.basename(r) for r in reps)
check(4, 'reports message-001..021 exist', all(n in have for n in need), str(len(reps)))
...
chain = {34: 'RFC-0033', 35: 'RFC-0034', 36: 'RFC-0035', 37: 'RFC-0036', 38: 'RFC-0037',
         ...
         44: 'RFC-0043', 45: 'RFC-0044', 46: 'RFC-0045', 47: 'RFC-0046'}
chain_ok, bad = True, []
...
check(5, 'Parent chain RFC-0034..0047 incl. 0043->0028 detour', chain_ok, str(bad))
...
check(5, 'no unqualified ratification claims for RFC-0044/0045/0047',
      all(not re.search(r'^\| RFC-00%d \|.*\*\*RATIFIED\*\*' % n, ri, re.M) for n in (44, 45, 47)))
st = open(f'{KB}/wiki/Source-Traceability.md', encoding='utf-8').read()
check(5, 'conflict C-11 recorded', '| C-11 |' in st)
check(5, 'duplicates D-64..D-68 recorded', all(('| D-%d |' % d) in st for d in range(64, 69)))
...
check(6, 'all 47 RFC documents carry a Parent header', not no_parent, str(no_parent))

# ---------- 7. Status & cross-page coherence ----------
readme = open(f'{KB}/README.md', encoding='utf-8').read()
check(7, 'README totals (21 msgs / 1138 snippets / 12 specs / 51 rfcs)',
      all(s in readme for s in ['21 messages processed', '**1138 code snippets**',
                                '**12 scaffolded documents in `specs/`**', '**51 files in `rfcs/`**']))
cl = open(f'{KB}/wiki/Changelog.md', encoding='utf-8').read()
miss_cl = [i for i in range(1, 22) if ('Message #%d' % i) not in cl]
check(7, 'changelog entries for messages 1..21', not miss_cl, str(miss_cl))

# ---------- 8. Traceability bookkeeping ----------
reg = sorted(set(int(m) for m in re.findall(r'^\| (\d+) \| 2026-', st, re.M)))
check(8, 'register rows 1..21 contiguous', reg == list(range(1, 22)), str(reg))
idx_count = len(re.findall(r'^#{2,3} Message #\d+ sub-message index', st, re.M))
check(8, 'sub-message indexes for msgs 2,3,5,8,10,12,14,16,18,21 (10)', idx_count == 10, str(idx_count))
xs = sorted(set(int(m) for m in re.findall(r'^\| X-(\d+) \|', st, re.M)))
check(8, 'cross-references contiguous X-01..X-94', xs == list(range(1, 95)), 'count=%d' % len(xs))
ds = sorted(set(int(m) for m in re.findall(r'^\| D-(\d+) \|', st, re.M)))
check(8, 'duplicate log contiguous D-1..D-68', ds == list(range(1, 69)), 'count=%d' % len(ds))
csids = sorted(set(int(m) for m in re.findall(r'^\| C-(\d+) \|', st, re.M)))
check(8, 'conflict log contiguous C-1..C-11', csids == list(range(1, 12)), str(csids))
