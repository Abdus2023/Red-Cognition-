#!/usr/bin/env python3
"""Message #23 final verification suite for the Red-Cognition knowledge base.
Run from the repository root: python3 knowledge-base/reports/message-023-verification-suite.py
Corpus scope: messages 1..23 processed, sub-messages [1]..[240], SN-001..SN-1348,
12 specs + 60 rfcs scaffolded files."""
...
EXPECTED_RANGES = {2: (1, 20), 3: (21, 40), 5: (41, 60), 8: (61, 80), 10: (81, 100),
                   12: (101, 120), 14: (121, 140), 16: (141, 160), 18: (161, 180), 21: (181, 200), 22: (201, 220), 23: (221, 240)}
SN_RANGES = {2: (1, 123), 3: (124, 212), 5: (213, 318), 8: (319, 427), 10: (428, 493),
             12: (494, 640), 14: (641, 825), 16: (826, 993), 18: (994, 1093), 21: (1094, 1138), 22: (1139, 1229), 23: (1230, 1348)}
...
check(2, 'all annex blocks byte-faithful vs archives (msgs 3..23)', annex_ok, str(bad))
...
need = ['message-%03d-report.md' % i for i in range(1, 24)]
have = set(os.path.basename(r) for r in reps)
check(4, 'reports message-001..023 exist', all(n in have for n in need), str(len(reps)))
...
check(5, 'Parent chain RFC-0034..0053 incl. 0043->0028 detour', chain_ok, str(bad))
...
check(5, 'no unqualified ratification claims for RFC-0044/0045/0048/0051/0053',
...
check(5, 'duplicates D-64..D-79 recorded', all(('| D-%d |' % d) in st for d in range(64, 80)))
...
terms23 = ['CMMS (RFC-0051)', 'CTVF (RFC-0052)', 'CRAIP (RFC-0053)', 'Cognitive Epoch (ratified)',
           'Architecture Governance Rule', 'MacroExpansionRecord', 'InvocationManifest', 'AgentManifest']
check(5, 'Glossary contains message-#23 terms (8/8)', all(x in gl for x in terms23),
      str([x for x in terms23 if x not in gl]))
...
check(7, 'README totals (23 msgs / 1348 snippets / 12 specs / 60 rfcs)',
      all(s in readme for s in ['23 messages processed', '**1348 code snippets**',
...
check(7, 'changelog entries for messages 1..23', not miss_cl, str(miss_cl))

# ---------- 8. Traceability bookkeeping ----------
reg = sorted(set(int(m) for m in re.findall(r'^\| (\d+) \| 2026-', st, re.M)))
check(8, 'register rows 1..23 contiguous', reg == list(range(1, 24)), str(reg))
idx_count = len(re.findall(r'^#{2,3} Message #\d+ sub-message index', st, re.M))
check(8, 'sub-message indexes for msgs 2,3,5,8,10,12,14,16,18,21,22,23 (12)', idx_count == 12, str(idx_count))
