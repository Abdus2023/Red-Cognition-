#!/usr/bin/env python3
"""Message #31 final verification suite for the Red-Cognition knowledge base.
Run from the repository root: python3 knowledge-base/reports/message-031-verification-suite.py
Corpus scope: KB messages 1..31 processed, sub-messages [1]..[360], SN-001..SN-2122,
...
TRANSCRIPT_MSGS = [2, 3, 5, 8, 10, 12, 14, 16, 18, 21, 22, 23, 25, 26, 27, 29, 30, 31]
MSG_PARTS = {2: 2, 3: 4, 5: 5, 8: 5, 10: 5, 12: 5, 14: 5, 16: 5, 18: 5, 21: 5, 22: 5, 23: 5, 25: 5, 26: 5, 27: 5, 29: 5, 30: 5, 31: 5}
EXPECTED_RANGES = {2: (1, 20), 3: (21, 40), 5: (41, 60), 8: (61, 80), 10: (81, 100),
                   12: (101, 120), 14: (121, 140), 16: (141, 160), 18: (161, 180),
                   21: (181, 200), 22: (201, 220), 23: (221, 240), 25: (241, 260), 26: (261, 280), 27: (281, 300), 29: (301, 320), 30: (321, 340), 31: (341, 360)}
SN_RANGES = {2: (1, 123), 3: (124, 212), 5: (213, 318), 8: (319, 427), 10: (428, 493),
             12: (494, 640), 14: (641, 825), 16: (826, 993), 18: (994, 1093),
             21: (1094, 1138), 22: (1139, 1229), 23: (1230, 1348), 25: (1349, 1419), 26: (1420, 1591), 27: (1592, 1777), 29: (1778, 1975), 30: (1976, 2094), 31: (2095, 2122)}
FENCED = {2: 120, 3: 89, 5: 106, 8: 109, 10: 66, 12: 147, 14: 185, 16: 168, 18: 100,
          21: 45, 22: 91, 23: 119, 25: 71, 26: 172, 27: 186, 29: 198, 30: 119, 31: 28}
...
check(1, 'all archive files exist (88 files)', not missing, str(missing))
...
check(1, 'sub-message labels [1]..[360] contiguous (360/360)', labels == list(range(1, 361)), 'count=%d' % len(labels))
check(1, 'every sub-message header carries a speaker label', speaker_ok)
hdr31 = re.compile(r'^## \[(\d+)\] (.*)$', re.M)
def sub_body(msg, n):
    ms = list(hdr31.finditer(archive_text[msg]))
    for i, m in enumerate(ms):
        if int(m.group(1)) == n:
            ...
            return b
    return None
check(1, 'D-99 verified in archive: [341]=[343]=[345]=[347]=[339] byte-identical',
      sub_body(31, 341) == sub_body(31, 343) == sub_body(31, 345) == sub_body(31, 347) == sub_body(30, 339))
check(1, 'D-100 verified in archive: [353]=[355]=[357]=[359] byte-identical; revisions [349]/[351] distinct',
      sub_body(31, 353) == sub_body(31, 355) == sub_body(31, 357) == sub_body(31, 359)
      and sub_body(31, 349) != sub_body(31, 351) != sub_body(31, 353))
...
check(2, 'SN-001..SN-2122 all present in Code-Snippets', tok_ok, str(miss[:5]))
tot = re.search(r'\*\*Corpus totals: (\d+) snippets\*\*', cs)
check(2, 'Code-Snippets corpus totals line = 2122', tot and tot.group(1) == '2122', tot.group(1) if tot else 'missing')
...
check(2, 'totals line has exactly one parenthetical breakdown (no duplicate residue)',
      tot_line.count('). (') == 0 and tot_line.endswith('Message #31 Annex at the bottom of this page).'),
...
for m in (16, 18, 21, 22, 23, 25, 26, 27, 29, 30, 31):
...
check(2, 'archive fenced total = 2119 (2122 - 3 inline msg#2)', len(all_blocks) == 2119, str(len(all_blocks)))
fenced_ok, fdet = True, []
for m in TRANSCRIPT_MSGS:
    if len(archive_blocks[m]) != FENCED[m]:
        fenced_ok = False; fdet.append('msg%d:%d!=%d' % (m, len(archive_blocks[m]), FENCED[m]))
check(2, 'per-message fenced counts = breakdown sums (all 18 transcript messages)', fenced_ok, str(fdet))
seq_ok, det = True, []
for m in (16, 18, 21, 22, 23, 25, 26, 27, 29, 30, 31):
    lo, hi = SN_RANGES[m]
...
check(2, 'annex SN sequences complete & ascending (msg#16/18/21/22/23/25/26/27/29/30/31)', seq_ok, str(det))
annex_ok, bad = True, []
for m in (3, 5, 8, 10, 12, 14, 16, 18, 21, 22, 23, 25, 26, 27, 29, 30, 31):
    hdr = re.search(r'## Message #%d Annex' % m, cs)
...
check(2, 'all annex blocks byte-faithful vs archives (msgs 3..31, 2119 blocks)', annex_ok, str(bad))
...
check(4, '2119/2119 archived fenced blocks verbatim in Wiki', missn == 0,
...
need = ['message-%03d-report.md' % i for i in range(1, 32)]
have = set(os.path.basename(r) for r in reps)
check(4, 'reports message-001..031 exist', all(n in have for n in need), str(len(reps)))
...
# ---------- 5. Normative consistency (message-#31 material) ----------
rec72 = open('rfcs/RFC-0072-ratification-record.md', encoding='utf-8').read()
check(5, 'RFC-0072 record scaffold: final revised form per [353] (dependency note + Registry Governance + Evolution Policy + Change Control + Conformance Profile)',
      '**Status:** **Ratified**' in rec72 and 'is hereby ratified as a normative specification' in rec72
      and 'Note on parent dependency' in rec72 and '### Registry Governance' in rec72
      and '### Protocol Evolution Policy' in rec72 and '### Change Control' in rec72
      and '### Conformance Profile (Minimal Interoperable Implementation)' in rec72)
check(5, 'RFC-0072 record provenance: origin [353]; D-99/D-100 + C-19 + X-140 + stray-parenthesis quirk noted',
      'sub-message [353]' in rec72[:3000] and 'D-99' in rec72[:3000] and 'D-100' in rec72[:3000]
      and 'C-19' in rec72[:3000] and 'X-140' in rec72[:3000] and 'missing opening parentheses' in rec72[:3000])
check(5, 'RFC-0072 record body matches archive sub-message [353] exactly',
      rec72[rec72.find('-->')+3:].strip() in archive_text[31] or norm(rec72[rec72.find('-->')+3:].strip()) in norm(archive_text[31]))
check(5, 'RFC-Index: Message #31 additions section with revision arc and C-19 extension',
      'Message #31 additions' in ri and 'record scaffold updated from [339]' in ri.replace('`', '').replace('*', '') or
      ('Message #31 additions' in ri and '[353]' in ri and 'C-19 (extended)' in ri))
check(5, 'C-19 extended in conflict log to cover revised tables (RFC-0060 Candidate clause)',
      '| C-19 |' in st and '[349]–[359]' in st and '"Candidate" although ratified ([285])' in st)
check(5, 'duplicates D-99 and D-100 recorded', '| D-99 |' in st and '| D-100 |' in st)
check(5, 'cross-references X-138..X-140 present', all(('| X-%d |' % x) in st for x in range(138, 141)))
terms31 = ['Registry Governance (RFC-0072 record)', 'Protocol Evolution Policy (RFC-0072 record)',
           'Change Control (RFC-0072 record)', 'Conformance Profile — CRCP Minimal Interoperable Implementation',
           'Provisional Ratification (deferred effectiveness)']
check(5, 'Glossary contains message-#31 governance terms (5/5)', all(x in gl for x in terms31),
      str([x for x in terms31 if x not in gl]))
rows72 = re.findall(r'^\| RFC-0072 \|.*$', ri, re.M)
check(5, 'RFC-0072 RFC-Index row still RATIFIED; ratified set unchanged (no new ratifications in msg#31)',
      bool(rows72) and any('RATIFIED' in r for r in rows72) and 'Ratified set after message #31' not in ri)
...
check(7, 'README: 31 messages processed; current totals (2122 / 12 specs / 86 rfcs)',
      '31 messages processed' in readme and all(s in readme for s in [
          '**2122 code snippets**', '**12 scaffolded documents in `specs/`**', '**86 files in `rfcs/`**']))
check(7, 'README: no stale messages-processed counts (28/29/30) remain',
      all(('%d messages processed' % n) not in readme for n in (28, 29, 30)))
check(7, 'README: message ordering #29 -> #30 -> #31 with totals ascending',
      readme.index('Message #29 = ') < readme.index('Message #30 = ') < readme.index('Message #31 = ')
      and readme.index('**2094 code snippets**') < readme.index('**2122 code snippets**'))
check(7, 'README Code Snippets table row current (2122 snippets, SN-001…SN-2122)',
      'ledger of all 2122 snippets (SN-001…SN-2122)' in readme and 'ledger of all 2094 snippets' not in readme)
check(7, 'Code-Snippets provenance header current (covers messages #2..#31)',
      '#29, #30, #31' in cs[:700] and 'message-031-part1..5' in cs[:1200])
miss_cl = [i for i in range(1, 32) if ('Message #%d' % i) not in cl]
check(7, 'changelog entries for messages 1..31', not miss_cl, str(miss_cl))

# ---------- 8. Traceability bookkeeping ----------
reg = sorted(set(int(m) for m in re.findall(r'^\| (\d+) \| 2026-', st, re.M)))
check(8, 'register rows 1..31 contiguous', reg == list(range(1, 32)), str(reg))
idx_count = len(re.findall(r'^#{2,3} Message #\d+ sub-message index', st, re.M))
check(8, 'sub-message indexes for the 18 transcript messages', idx_count == 18, str(idx_count))
xs = sorted(set(int(m) for m in re.findall(r'^\| X-(\d+) \|', st, re.M)))
check(8, 'cross-references contiguous X-01..X-140', xs == list(range(1, 141)), 'count=%d' % len(xs))
ds = sorted(set(int(m) for m in re.findall(r'^\| D-(\d+) \|', st, re.M)))
check(8, 'duplicate log contiguous D-1..D-100', ds == list(range(1, 101)), 'count=%d' % len(ds))
