#!/usr/bin/env python3
"""Deep audit suite #6 (KB message #24, directive 'Deeply Verification').
Run from the repository root: python3 knowledge-base/reports/message-024-verification-suite.py
Corpus scope: messages 1..24 processed, sub-messages [1]..[240], SN-001..SN-1348,
12 specs + 60 rfcs scaffolded files (72 scaffolds)."""
...
check(1, 'all archive files exist (58 files)', not missing, str(missing))
...
check(1, 'sub-message labels [1]..[240] contiguous (240/240)', labels == list(range(1, 241)), 'count=%d' % len(labels))
...
check(2, 'SN-001..SN-1348 all present in Code-Snippets', tok_ok, str(miss[:5]))
tot = re.search(r'\*\*Corpus totals: (\d+) snippets\*\*', cs)
check(2, 'Code-Snippets corpus totals line = 1348', tot and tot.group(1) == '1348', tot.group(1) if tot else 'missing')
ledger_rows = re.findall(r'^\| (SN-\d{3}) \|', cs, re.M)
check(2, 'message #2 ledger has 123 rows', len(ledger_rows) == 123, str(len(ledger_rows)))
for m in (16, 18, 21, 22, 23):
...
check(2, 'archive fenced total = 1345 (1348 - 3 inline msg#2)', len(all_blocks) == 1345, str(len(all_blocks)))
...
check(2, 'annex SN sequences complete & ascending (msg#16/18/21/22/23)', seq_ok, str(det))
annex_ok, bad = True, []
for m in (3, 5, 8, 10, 12, 14, 16, 18, 21, 22, 23):
    lo, hi = SN_RANGES[m]
    hdr = re.search(r'## Message #%d Annex' % m, cs)
...
check(2, 'all annex blocks byte-faithful vs archives (msgs 3..23, 1345 blocks)', annex_ok, str(bad))
...
check(3, 'rfcs/ = 60 files', len(rfcs) == 60, str(len(rfcs)))
...
check(3, 'RFC-0001..0053 exactly once each', ids == list(range(1, 54)), '%d docs' % len(rfc_docs))
rec_ids = sorted(int(re.match(r'RFC-(\d{4})', f).group(1)) for f in records)
check(3, 'ratification records = 0001, 0002, 0011, 0042, 0049, 0050, 0052', rec_ids == [1, 2, 11, 42, 49, 50, 52], str(rec_ids))
...
check(3, 'all 72 scaffolds carry KB provenance headers', prov_ok)
check(3, 'all 72 scaffold bodies verbatim from archive', faithful == 72,
...
check(4, '1345/1345 archived fenced blocks verbatim in Wiki', missn == 0,
...
need = ['message-%03d-report.md' % i for i in range(1, 25)]
have = set(os.path.basename(r) for r in reps)
check(4, 'reports message-001..024 exist', all(n in have for n in need), str(len(reps)))
...
# ---------- 5. Normative consistency (message-#23 material + ratified set) ----------
r50 = open('rfcs/RFC-0050-architecture-conformance-specification.md', encoding='utf-8').read()
check(5, 'RFC-0050 scaffold: governance rule + ConformanceManifest + Cognitive Epoch',
      'Architecture Governance Rule' in r50 and 'ConformanceManifest' in r50 and 'Cognitive Epoch' in r50)
check(5, 'RFC-0050 scaffold provenance cites [221] and ratification [224]/[225]',
      'sub-message [221]' in r50[:800] and '[224]' in r50[:800] and '[225]' in r50[:800])
rec50 = open('rfcs/RFC-0050-ratification-record.md', encoding='utf-8').read()
check(5, 'RFC-0050 ratification record: Ratified + constitutional architecture',
      'Ratified' in rec50 and 'constitutional architecture specification' in rec50)
r52 = open('rfcs/RFC-0052-ctvf-testing-verification.md', encoding='utf-8').read()
check(5, 'RFC-0052 scaffold v1.2: MUST-grade conformance + verification pipeline',
      'A conforming CTVF implementation **MUST** provide' in r52 and 'Proof Verification' in r52)
rec52 = open('rfcs/RFC-0052-ratification-record.md', encoding='utf-8').read()
check(5, 'RFC-0052 ratification record: "hereby ratified as a normative specification"',
      'hereby ratified as a normative specification' in rec52)
r53 = open('rfcs/RFC-0053-craip-remote-agent-invocation.md', encoding='utf-8').read()
check(5, 'RFC-0053 scaffold: state machine + AgentManifest + RemoteError; stray paren preserved',
      'Protocol State Machine' in r53 and 'AgentManifest' in r53 and 'RemoteError' in r53
      and 'RemoteInvocationFailed`)' in r53)
check(5, 'RFC-0053 provenance: Candidate status + not ratified + typo quirk noted',
      'Candidate for Ratification' in r53[:900] and 'No ratification decision' in r53[:900]
      and 'RemoteInvocationFailed' in r53[:900])
r51 = open('rfcs/RFC-0051-cmms-macro-metaprogramming.md', encoding='utf-8').read()
check(5, 'RFC-0051 scaffold: MacroExpansionRecord + Draft status in provenance',
      'MacroExpansionRecord' in r51 and '(Draft)' in r51[:700])
row_ok = True
for n, pat in ((50, 'RATIFIED'), (52, 'RATIFIED')):
    rows = re.findall(r'^\| RFC-%04d \|.*$' % n, ri, re.M)
    if not rows or not any(pat in r for r in rows): row_ok = False
check(5, 'RFC-Index rows RFC-0050 & RFC-0052 = RATIFIED', row_ok)
check(5, 'ratified set includes RFC-0050 and RFC-0052',
      'RFC-0050 (Architecture & Conformance v1.2' in ri and 'RFC-0052 (CTVF v1.2, ratified per [235])' in ri)
check(5, 'duplicates D-75..D-79 recorded (incl. D-76 identical re-send)',
      all(('| D-%d |' % d) in st for d in range(75, 80)) and '[223]' in st)
check(5, 'cross-references X-101..X-105 present', all(('| X-%d |' % x) in st for x in range(101, 106)))

# ---------- 6. RFC parent-chain integrity ----------
no_parent = [f for f in rfc_docs if '**Parent:**' not in open('rfcs/' + f, encoding='utf-8').read()[:1500]]
check(6, 'all 53 RFC documents carry a Parent header', not no_parent, str(no_parent))
...
check(6, 'documented parent chain RFC-0034..0053 exact (incl. 0043->0028 detour)', chain_ok, str(badc))

# ---------- 7. Status & cross-page coherence ----------
check(7, 'README totals (24 msgs / 1348 snippets / 12 specs / 60 rfcs)',
      all(s in readme for s in ['24 messages processed', '**1348 code snippets**',
                                '**12 scaffolded documents in `specs/`**', '**60 files in `rfcs/`**']))
miss_cl = [i for i in range(1, 24) if ('Message #%d' % i) not in cl]
check(7, 'changelog entries for messages 1..23 (incl. directives)', not miss_cl, str(miss_cl))
check(7, 'no unqualified ratification claims for RFC-0044/0045/0048/0051/0053',
      all(not re.search(r'^\| RFC-00%d \|.*\*\*RATIFIED\*\*' % n, ri, re.M) for n in (44, 45, 48, 51, 53)))
check(7, 'RFC-Index constitutional governance section present',
      'Constitutional governance (RFC-0050 \u00a717, ratified)' in ri)
check(7, 'conflict log coherent (C-1..C-12; no C-13 created for preserved typo)',
      '| C-12 |' in st and '| C-13 |' not in st)

# ---------- 8. Traceability bookkeeping ----------
reg = sorted(set(int(m) for m in re.findall(r'^\| (\d+) \| 2026-', st, re.M)))
check(8, 'register rows 1..24 contiguous', reg == list(range(1, 25)), str(reg))
idx_count = len(re.findall(r'^#{2,3} Message #\d+ sub-message index', st, re.M))
check(8, 'sub-message indexes for the 12 transcript messages', idx_count == 12, str(idx_count))
xs = sorted(set(int(m) for m in re.findall(r'^\| X-(\d+) \|', st, re.M)))
check(8, 'cross-references contiguous X-01..X-105', xs == list(range(1, 106)), 'count=%d' % len(xs))
ds = sorted(set(int(m) for m in re.findall(r'^\| D-(\d+) \|', st, re.M)))
check(8, 'duplicate log contiguous D-1..D-79', ds == list(range(1, 80)), 'count=%d' % len(ds))
